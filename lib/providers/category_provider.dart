...
class CategoryProvider extends ChangeNotifier {
  ...
  List<FileSystemEntity> documents = List();
  List<String> documentsTabs = List();
  ...
  getDocuments(String type) async {
    documentLoading = true;
    setLoading(true);
    documents.clear();
    documentsTabs.clear();
    documentsTabs.add("All");
    List<FileSystemEntity> files =
        await FileUtils.getAllFiles(showHidden: showHidden);
    files.forEach((file) {
      String mimeType = mime(file.path);
      ...
      if (mimeType != null) {
        if (mimeType.split("/")[0] == type) {
          ...
          documentsTabs
              .add("${file.path.split("/")[file.path.split("/").length - 2]}");
          documentsTabs = documentsTabs.toSet().toList();
        }
        notifyListeners();
      }
    });
